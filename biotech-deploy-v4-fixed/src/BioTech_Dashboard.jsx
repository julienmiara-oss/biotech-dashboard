...
export default function BioTechDashboard() {
  ...
  // === DATA NORMALIZATION — merge old Supabase data with defaults ===
  const settings = {
    ...initialSettings,
    ...(shared.settings || {}),
    delays: { ...initialSettings.delays, ...(shared.settings?.delays || {}) },
    fundraising: { ...initialSettings.fundraising, ...(shared.settings?.fundraising || {}) },
    alertThresholds: { ...initialSettings.alertThresholds, ...(shared.settings?.alertThresholds || {}) },
  };
  const entities = shared.entities || initialEntities;
  const employees = (shared.employees || initialEmployees).map(e => ({
    ticketsResto: 0, transportBenefit: 0, bonus: 0, plannedStartDate: null, ...e
  }));
  const fteAlloc = shared.fteAlloc || initialFteAlloc;
  const revenue = Object.fromEntries(
    Object.entries(shared.revenue || initialRevenue).map(([k, arr]) => [k, expandArray(arr || [], TOTAL_MONTHS)])
  );
  const overhead = Object.fromEntries(
    Object.entries(shared.overhead || initialOverhead).map(([k, arr]) => [k, expandArray(arr || [], TOTAL_MONTHS)])
  );
  const projects = (shared.projects || initialProjects).map(p => ({ growthRate: 0, ...p }));
  const cashBalance = shared.cashBalance ?? 1500000;
  const overheadAlloc = shared.overheadAlloc || overheadKeys;
  const raises = shared.raises || initialRaises;
  const scenarios = (shared.scenarios || initialScenarios).map(sc => ({
    revenueMultiplier: 1.0, costMultiplier: 1.0, headcountAdj: 0, delayMonths: 0, notes: "", color: "#2E5090", ...sc
  }));
  ...
  const activeScenario = scenarios.find(s => s.id === activeScenarioId) || scenarios[0] || { id: "base", name: "Base", revenueMultiplier: 1.0, costMultiplier: 1.0, headcountAdj: 0, delayMonths: 0, notes: "", color: "#2E5090" };
  ...
  const computed = useMemo(() => {
    ...
    const alerts = [];
    const thresholds = settings.alertThresholds || { minRunway: 12, minRunwayCritical: 6, maxBurnRate: 500000, maxVariance: 0.10 };
    if (runway < (thresholds.minRunwayCritical || 6)) alerts.push({ level: "critical", text: `Runway critique: ${runway.toFixed(1)} mois` });
    else if (runway < (thresholds.minRunway || 12)) alerts.push({ level: "warning", text: `Runway faible: ${runway.toFixed(1)} mois` });
    if (avgBurn > (thresholds.maxBurnRate || 500000)) alerts.push({ level: "warning", text: `Burn rate élevé: ${fmt(avgBurn)}` });
        ...
        {Object.entries(projectsByEntity).map(([entId, projs]) => {
          ...
          return (
            <div key={entId} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              <h3 className="text-sm font-bold text-gray-700 mb-4 flex items-center gap-2"><div className="w-3 h-3 rounded-full" style={{backgroundColor: ent.color || "#999"}} />{ent.name || "Entité inconnue"}</h3>
              {projs.map(p => {
          ...
          {scenarios.map(sc => (
            <div key={sc.id} className={`bg-white rounded-xl shadow-sm border p-5 cursor-pointer transition ${activeScenarioId === sc.id ? "border-blue-500 bg-blue-50" : "border-gray-100"}`} onClick={() => setActiveScenarioId(sc.id)}>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm mb-3">
                <div><span className="text-gray-600">Revenus:</span> <strong>×{(sc.revenueMultiplier ?? 1.0).toFixed(2)}</strong></div>
                <div><span className="text-gray-600">Coûts:</span> <strong>×{(sc.costMultiplier ?? 1.0).toFixed(2)}</strong></div>
                <div><span className="text-gray-600">HC:</span> <strong>{(sc.headcountAdj || 0) > 0 ? "+" : ""}{sc.headcountAdj || 0}</strong></div>
                <div><span className="text-gray-600">Délai:</span> <strong>{sc.delayMonths || 0} mois</strong></div>
              </div>
